 ...
 *
 * PHILOSOPHY: Collect raw data only. All processing happens in Node.js for better performance and maintainability.
 */

export const brandingScript = `
/**
 * Extracts raw brand data from the page.
 * Returns minimal data - all processing done in Node.js
 */
(async function __extractBrandDesign() {
  const toPx = v => {
    ...
    if (v.endsWith("rem"))
      return parseFloat(v) * parseFloat(getComputedStyle(document.documentElement).fontSize || 16);
    if (v.endsWith("em"))
      return parseFloat(v) * parseFloat(getComputedStyle(document.body).fontSize || 16);
  ...
  const resolveSvgStyles = svg => {
    ...
    const applyResolvedStyle = (clonedEl, originalEl, computed, prop) => {
      ...
      if (attrValue && attrValue.includes("var(")) {
        ...
        const isExplicit = originalEl.hasAttribute(prop) || originalEl.style[prop];
        ...
        try {
          if (rule.type === CSSRule.STYLE_RULE) {
            ...
            ["border-radius", "border-top-left-radius", "border-top-right-radius",
             "border-bottom-left-radius", "border-bottom-right-radius"].forEach(p => {
            ...
            ["margin", "margin-top", "margin-right", "margin-bottom", "margin-left",
             ...
             "gap", "row-gap", "column-gap"].forEach(p => {
              const v = toPx(s.getPropertyValue(p));
  ...
  const getStyleSnapshot = el => {
    ...
    const fontStack = cs.getPropertyValue("font-family")
      ?.split(",")
  ...
  const findImages = () => {
    ...
    const headerLinkImg = document.querySelector('header a img, header a svg, nav a img, nav a svg, [role="banner"] a img, [role="banner"] a svg, .header a img, .header a svg');

    if (headerLinkImg) {
      if (headerLinkImg.tagName.toLowerCase() === "svg") {
        const resolvedSvg = resolveSvgStyles(headerLinkImg);
        const serializer = new XMLSerializer();
        const svgStr = "data:image/svg+xml;utf8," + encodeURIComponent(serializer.serializeToString(resolvedSvg));
      ...
      const logoImgCandidates = Array.from(document.images)
        .filter(img => /logo/i.test(img.alt || "") || /logo/i.test(img.src) || img.closest('[class*="logo"]'))
        .filter(img => !img.closest('[class*="testimonial"], [class*="client"], [class*="partner"]'));
      ...
      const svgLogoCandidates = Array.from(document.querySelectorAll("svg"))
        .filter(s => /logo/i.test(s.id) || /logo/i.test(s.className?.baseVal || ""))
        .filter(svg => !svg.closest('[class*="testimonial"], [class*="client"], [class*="partner"]'));
  ...
  const detectColorScheme = () => {
    ...
    const bg = getComputedStyle(body).backgroundColor || getComputedStyle(html).backgroundColor;
